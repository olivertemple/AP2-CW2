...
sql.connect(config, async (err) => {
    if (err) {
        ...
        app.use((req, res, next) => {
            ...
            res.on("finish", () => {
                if (res.statusCode == 500) {
                    logger.error(
                        `${res.statusCode} ${req.method} ${req.originalUrl} id=${req.rid} error=${intercepted_body}`
                    );
                } else {
                    logger.info(
                        `${res.statusCode} ${req.method} ${req.originalUrl} id=${req.rid}`
                    );
                }
            });
            next();
        ...
        app.listen(port, (err) => {
            err
                ? logger.error(`STARTUP startup error - ${err}`)
